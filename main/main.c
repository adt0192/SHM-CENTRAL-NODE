...
esp_err_t init_2d_arrays()
{
    ...
    ESP_LOGW(TAG, "Free heap memmory (bytes): <%zu>", xPortGetFreeHeapSize());
    ESP_LOGW(TAG, "*********************************************************");
...
static void check_header_incoming_data_task(void *pvParameters)
{
    ...
    while (1)
    {
            ...
            (in_transaction_ID_dec == MSG_COUNTER_RX - 1))
        {
            ...
            ESP_LOGE(TAG,
                     "******************** FREE HEAP MEMORY ********************");
            ESP_LOGW(TAG, "<%zu> BYTES", xPortGetFreeHeapSize());
            ESP_LOGE(TAG,
                     "******************** FREE HEAP MEMORY ********************");
            ///// VISUALIZE
            ///**********************************************************
    ...
    while (1)
    {
        ...
        if (xQueueReceive(uart_queue, (void *)&event, portMAX_DELAY))
        {
            ...
            *********************"); ESP_LOGI(TAG, "Heap integrety %i",
            ...
            ESP_LOGI(TAG, "FreeHeapSize: %zu", xPortGetFreeHeapSize()); // Added
